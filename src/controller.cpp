...
Controller::U_s Controller::cal_Rd_thrust(const PID_ctrl<cmd_s,State_s>::res_s &in_ctrl_res) {//计算目标姿态与目标油门
    ...
    if (ros::Time::now() - ctrl_res.header < ros::Duration(0.5f)) {
        ...
        /* get U1 */
        double real_U1 = - ctrl_res.res.transpose() * _R.col(2);//计算目标加速度向量在机体系Z轴上的投影
        res.U1 = real_U1 / ONE_G * ctrl_core.get_hover_thrust();//得到油门值
        ...
        if ( fabsf(_body_z(2)) > 0.0001f ) {
            _body_x = _y_C.cross(_body_z);//_y_C 叉乘 _body_z
            if (_body_z(2) < 0) {
                _body_x = - _body_x;
            }
...
bool Controller::takeoff_land_srv_handle(quad_pos_ctrl::SetTakeoffLand::Request& req,
    ...
    if (req.takeoff) {
        ...
        set_hover_pos(pos_d,yaw_d);
        Eigen::Vector3d euler_pix;
        Eigen::Quaterniond q_pix;
        mavros_interface.get_ahrs(q_pix);
        get_euler_from_q(euler_pix, q_pix);
        ...
        while(ros::ok() && ros::Time::now()-start_land_task_time < ros::Duration(8.0)) {
            ...
            if (temp_lidar_z < 0.25f && temp_lidar_z > 0.001f && fabs(state_now.Vel(2)) < 1.0f) {//加入lidar数据大于0的判据，防止lidar失效空中锁定电机
                ROS_INFO("detect land: disarm");
                // sleep(1);
                arm_disarm_vehicle(false); // disarm uav
                break;
            }
            land_loop.sleep();
        }
