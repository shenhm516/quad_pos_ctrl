...
class Mavros_Interface {
    public:
        ...
        _nh("~cmd") {
            ...
            std::string AHRS_sub_name;
            state_sub_name = base_name + "/imu/data";
            ahrs_sub = _nh.subscribe(state_sub_name.c_str(),10,&Mavros_Interface::ahrs_cb,this);
        ...
        void ahrs_cb(const sensor_msgs::ImuConstPtr& ahrs_msg) {            
            pix_AHRS.header        = ahrs_msg->header;
            pix_AHRS.orientation.w = ahrs_msg->orientation.w;
            pix_AHRS.orientation.x = ahrs_msg->orientation.x;
            pix_AHRS.orientation.y = ahrs_msg->orientation.y;
            pix_AHRS.orientation.z = ahrs_msg->orientation.z;
        }

        void get_ahrs (Eigen::Quaterniond & ahrs){
            ahrs.w()  = pix_AHRS.orientation.w;
            ahrs.x()  = pix_AHRS.orientation.x;
            ahrs.y()  = pix_AHRS.orientation.y;
            ahrs.z()  = pix_AHRS.orientation.z;
        }
